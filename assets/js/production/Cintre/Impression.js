$(document).ready(function() {
    $("#show_modal").on("click", function(event) {
        ...
        $("form").attr("action", "save_cintre_impression_data");
        $("input").val("");
        $("textarea").val("");
        $("#IN_MACHINE").val("IMPRESSION INJECTION 1");
        $("#QUART_TIME").val("J");
        $("#modal_form_cintre_impression").modal("show");


    });
    $('.operateur').autocomplete({
        source: base_url + "Production/autocomplete_operateur",
        appendTo: "#modal_form_cintre_impression"
    });
    ...
    table = $("#dataTable").DataTable({
        processing: true,
        ajax: base_url + "Production/get_data_list_cintre_impression",
        language: {
    ...
    $('form').on('submit', function(event) {
        ...
        $.ajax({
            ...
            error: function(data) {
                $("#modal_form_cintre_impression").modal("hide");
    ...
    $("#show_data").on("click", function(event) {
        ...
        let link = base_url + "production/get_data_list_cintre_impression?Quart_show=" + Quart_show + "&Operateur_show=" + Operateur_show + "&machine_show=" + machine_show + "&date_debut_show=" + date_debut_show + "&date_fin_show=" + date_fin_show;
        table.ajax.url(link);
        table.ajax.reload();
    ...
    function cintre_injection_fonction() {
        $(".edit_cintre_injection").on("click", function(event) {
            ...
            $.post(base_url + "production/get_data_cintre_injection", { refnum }, function(data) {
                $("form").attr("action", "update_cintre_injection");
        ...
        $(".delete_cintre_injection").on("click", function(event) {
            event.preventDefault();
            let refnum = $(this).attr("id");
            $.post(base_url + "production/delete_date_cintre_impression", { refnum }, function(data) {

            }).done(() => {
                table.ajax.reload();
                swal("Succè! ", "Injection supprimer.", {
                    icon: "success",
                    buttons: {
                        confirm: {
